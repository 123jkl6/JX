...
function traverse_with_json(jsonFile){
	...
	fs.readFile(jsonFile,function(err,data){
		...
		runCheck(check,jsonDir,resultsFile,recepients);
	});	
}

async function runCheck(check,jsonFile,resultsFile,recepients){
    for (var oneFile of check){
        ...
        await findKeywords(jsonFile,relPath).then(function(stringIn){
...
async function findKeywords(jsonFile,relPath){
    var filename = path.resolve(jsonFile,relPath);
